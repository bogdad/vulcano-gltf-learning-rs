...
fn grad(hash_val: usize, x: f32, y: f32, z: f32) -> f32
{
  ...
  let v: f32 = if h < 4 {y} else if h == 12 || h == 14 {x} else {z};
  ...
  lerp(
      ...
      lerp(v,
           lerp(u,
                ...
                grad(hash[BA], x - 1.0, y, z)), /* BLENDED */
           lerp(u,
                grad(hash[AB], x, y - 1.0, z),       /* RESULTS */
                grad(hash[BB], x - 1.0, y - 1.0, z))), /* FROM  8 */
      lerp(v,
           lerp(u,
                grad(hash[AA + 1], x, y, z - 1.0),      /* CORNERS */
                grad(hash[BA + 1], x - 1.0, y, z - 1.0)), /* OF CUBE */
           lerp(u, grad(hash[AB + 1], x, y - 1.0, z - 1.0), 
...
pub fn hetero_terrain_new_perlin(xi: f32, yi: f32, zi:f32, H:f32, 
	lacunarity: f32, octaves: f32, offset: f32, noisebasis: i32) -> f32 {
