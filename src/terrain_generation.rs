...
*/
	let (origin_x, origin_y, origin_z) = if rseed == 0 {
		...
		(( 0.5 - v[0] ) * 1000.0, ( 0.5 - v[1] ) * 1000.0, ( 0.5 - v[2] ) * 1000.0)
    };
...
*/
	...
	if vertIdx1.len() < 2 && vertIdx2.len() < 2 {
		panic!("lengths cant be both less than 2 but they are {:?} {:?}", vertIdx1.len(), vertIdx2.len());
	}
	let mut fan = false;
    if vertIdx1.len() != vertIdx2.len() {
        if vertIdx1.len() == 1 && vertIdx2.len() > 1 {
            ...
            panic!("if len first is 1 len  the second should be more than 1 but they are {:?} {:?}", vertIdx1.len(), vertIdx2.len());
        }
    }
